...
function buildSymptomAdvice(input: SummaryInput) {
  ...
  if (/(?:아픽사반|엘리퀴스|와파린|리바록사반|자렐토|에독사반|다비가트란)/i.test(input.medication))
    return "배가 아픈 증상은 오메가-3 근거와 별도로 봐야 합니다. 통증이 계속되거나 심해지면 처방기관에 문의하세요. 검은변·혈변·토혈이 함께 있으면 바로 진료받으세요.";
  return "배가 아픈 증상은 보충제 근거와 별도로 봐야 합니다. 통증이 계속되거나 심해지면 의료기관에 문의하세요.";
}
