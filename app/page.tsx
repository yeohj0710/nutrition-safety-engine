...
export default function Home(){const perQuestion=Object.values(core.per_question);const coreRange=`질문마다 ${Math.min(...perQuestion)}~${Math.max(...perQuestion)}건`;const stats=[{label:"안전성 관련 문헌",value:manifest.records,note:"수집한 전체 문헌 중 성분과 안전성 결과가 함께 나온 문헌"},{label:"용량이 보고된 문헌",value:manifest.with_dose,note:"초록에 mg·IU 등 복용량이 나온 문헌"},{label:"무료 원문이 있는 문헌",value:manifest.with_fulltext_locator,note:"논문 본문을 바로 열 수 있는 문헌"},{label:"결과에 사용하는 문헌",value:core.core_records,note:coreRange}];return <main id="main-content" tabIndex={-1} data-scope="validated_thesis_scope" className="app-page min-h-screen px-4 pt-4 pb-24 sm:px-6 sm:pb-36 lg:px-6 lg:pb-52"><div className="page-shell flex flex-col gap-4">
 <section className="surface-card rounded-[1.15rem] px-4 py-4"><h1 className="text-[1.25rem] font-bold leading-snug text-foreground">항응고제 복용·신장질환 위험이 있을 때의 보충제 안전성</h1><p className="mt-1 text-sm leading-6 text-muted">복용량과 함께 먹는 약, 결석·신장 병력, 검사 결과를 문헌에 보고된 용량과 상호작용 결과와 비교해요.</p></section>
