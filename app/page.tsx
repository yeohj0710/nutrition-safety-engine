...
export default function Home(){const perQuestion=Object.values(core.per_question);const coreRange=`질문별 ${Math.min(...perQuestion)}~${Math.max(...perQuestion)}건`;const stats=[{label:"찾은 관련 문헌",value:manifest.records,note:"대상·성분·안전성 내용이 함께 확인된 자료"},{label:"용량을 확인한 문헌",value:manifest.with_dose,note:"초록에서 mg·IU 등 복용량을 확인한 자료"},{label:"원문 바로가기",value:manifest.with_fulltext_locator,note:"무료 공개 원문 위치가 연결된 자료"},{label:"중요 문헌",value:core.core_records,note:coreRange}];return <main id="main-content" tabIndex={-1} data-scope="validated_thesis_scope" className="app-page min-h-screen px-4 py-4 sm:px-6 lg:px-6"><div className="page-shell flex flex-col gap-4">
