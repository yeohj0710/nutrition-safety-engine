...
export default function Home(){const perQuestion=Object.values(core.per_question);const coreRange=`다섯 상황에 질문마다 ${Math.min(...perQuestion)}~${Math.max(...perQuestion)}건`;const stats=[{label:"근거 문헌",value:manifest.records,note:"AI가 선별한 뒤 근거 문장과 용량을 뽑아낼 수 있었던 PubMed 문헌"},{label:"용량이 보고된 문헌",value:manifest.with_dose,note:"초록에 mg·IU 등 복용량이 나온 문헌"},{label:"초록 근거 문장을 확인한 문헌",value:manifest.source_scope.abstract_only,note:`초록에서 근거 문장의 위치까지 확인한 문헌. 나머지 ${manifest.source_scope.title_only}건은 초록이 없어 제목을 위치로 남겼습니다.`},{label:"핵심 근거 문헌",value:core.core_records,note:coreRange}];return <main id="main-content" tabIndex={-1} data-scope="ai_selected_thesis_scope_v40" className="app-page min-h-screen px-4 pt-4 pb-24 sm:px-6 sm:pb-36 lg:px-6 lg:pb-52"><div className="page-shell flex flex-col gap-4">
 ...
 <section className="surface-card rounded-[1.15rem] px-4 py-5 sm:px-5" id="explorer"><PersonalizedSafetyQuery/></section>
 </div></main>}
