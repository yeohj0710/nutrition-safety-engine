...
describe("personalized safety API",()=>{
 ...
 it("addresses the user without system-oriented profile wording",async()=>{delete process.env.OPENAI_API_KEY;const response=await POST(new Request("http://local/api/personalized-safety",{method:"POST",body:JSON.stringify({ingredient:"칼슘",dose:"600 mg/day",condition:"칼슘옥살산 신장결석 병력",labs:"24시간 요중 칼슘 280 mg/day"})}));const body=await response.json();expect(body.ai_summary).toContain("칼슘 보충제를 600 mg/day 복용 중이시군요");expect(body.ai_summary).toContain("함께 살펴볼게요");expect(body.ai_summary).not.toContain("현재 입력한 조건");});
