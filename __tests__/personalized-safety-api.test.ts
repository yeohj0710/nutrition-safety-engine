...
describe("personalized safety API",()=>{
 ...
 it("summarizes the profile without system wording or conversational reactions",async()=>{delete process.env.OPENAI_API_KEY;const response=await POST(new Request("http://local/api/personalized-safety",{method:"POST",body:JSON.stringify({ingredient:"칼슘",dose:"600 mg/day",condition:"칼슘옥살산 신장결석 병력",labs:"24시간 요중 칼슘 280 mg/day"})}));const body=await response.json();expect(body.ai_summary).toContain("칼슘 보충제 600 mg/day 복용, 칼슘옥살산 신장결석 병력, 24시간 요중 칼슘 280 mg/day 검사 결과를 종합하면");expect(body.ai_summary).not.toMatch(/현재 입력한 조건|이시군요|살펴볼게요|적어주셨네요/);});
 ...
 it.each([
  ["missing input number","신장결석 병력이 있어 섭취원과 검사 결과를 함께 살펴야 합니다. 식이 칼슘과 보충제 칼슘을 구분해 확인하세요."],
  ["overlong paragraph",`600 mg/day ${"긴 설명 ".repeat(80)}`],
 ])("rejects %s and returns a concise fallback",async(_caseName,output_text)=>{process.env.OPENAI_API_KEY="test-key";vi.spyOn(globalThis,"fetch").mockResolvedValue(new Response(JSON.stringify({output_text}),{status:200,headers:{"content-type":"application/json"}}));const response=await POST(new Request("http://local/api/personalized-safety",{method:"POST",body:JSON.stringify({ingredient:"칼슘",dose:"600 mg/day",labs:"24시간 요중 칼슘 280 mg/day"})}));const body=await response.json();expect(body.ai_summary).toContain("600 mg/day");expect(body.ai_summary).toContain("280 mg/day");expect(body.ai_summary.length).toBeLessThanOrEqual(300);expect(body.ai_summary).not.toContain("긴 설명");});
});
