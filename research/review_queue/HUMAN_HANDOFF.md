...
Convenience copy: `external_review_handoff.xlsx` contains nine visually checked sheets for PRESS, duplicate context, registry context/decisions, KoreaMed–PubMed linkage, and the 50-row screening training pilot. It is not the authority of record; reconcile completed cells back to the canonical CSV files and run validators. Its source/workbook hashes are stored in `external_review_handoff_manifest.json`.
...
3. Screening and full text: adjudicate all 35 KoreaMed–PubMed candidate links with reason, verifier, and timestamp; complete the 50-row two-reviewer training pilot; complete primary decisions; run `build:secondary-screening`; complete the blinded secondary queue; then adjudicate and record lawful full-text access, exclusion reasons, and report-study links.

For PubMed workload distribution, filter `data/interim/screening_batch_assignments.csv` by `batch_id`. It partitions all 19,961 record-question units into 40 deterministic batches (maximum 500 rows), prioritizing proxy disagreements within each question. It contains no decision authority; write reviewed outcomes only to `screening_decisions.csv`.
