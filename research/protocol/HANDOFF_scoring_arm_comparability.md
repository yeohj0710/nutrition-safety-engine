...
> **결과 반영 (2026-07-30, 같은 날 늦게).** 권혁찬 채점 arm이 완료됐고
> (`research_v3/logs/v50_scoring_report.json`), 사전지정한 §5 규칙의 **세 번째 갈래**가
> 나왔다: 파이프라인 전수 retain 18.23%(7,875/43,207) 대 채점자 설계기반 추정 11.95%,
> 비 **0.66배**. 여형준(2.18배, 과소포함)과 **방향이 반대다.**
> 규칙대로 **어느 쪽으로도 설계 수준 결론을 내지 않는다.** 과소포함을 공유 선별 설계의
> 성질로 일반화할 수 없고, 여형준의 2.18배는 그 파이프라인에 국한된 특성으로 보는 것이
> 자료에 맞다(확정은 불가 — 코퍼스·질문·분류기·채점자가 모두 다르다).
>
> **연구자 결정: 교차 비교를 학위논문 범위에서 제외한다.** 두 연구 모두
> `clinical_recommendation: false`·`decision_authority: none`·`release_ready: false`이고
> 선별 정확도를 주장하지 않으므로 설계 수준 일반화가 필요하지 않다. 같은 이유로 사람
> 참조표준도 범위 밖으로 둔다. **단, 각 연구의 채점 arm 소견은 그대로 한계로 남는다** —
> 비교를 안 하는 것과 발견을 지우는 것은 다르다. 사람 참조표준이 0건이라는 사실이
> `_vs_ai_reference` 명명 규칙의 근거이므로 §5 명명 규율도 계속 유효하다.
>
> 아래 §2의 고정 항목 17개는 **앞으로 채점 arm을 다시 설계할 때의 체크리스트로** 남긴다.
