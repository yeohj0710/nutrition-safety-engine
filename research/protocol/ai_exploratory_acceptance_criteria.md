...
- [ ] 관찰값·초록값·AI 추론값을 분리한다.
- [ ] 모든 추출값에 원자료 해시와 locator를 연결한다.
- [ ] 기술 분포만 생성하고 효과 통합·RoB·GRADE를 수행하지 않는다.
- [ ] 접근하지 못한 자료를 근거 부재로 해석하지 않는다.

## 주장·엔진

- [ ] v2 잠정 주장이 `validated_thesis_scope`로 승격되지 않는다.
- [ ] 도구가 복용·용량·중단·의뢰 임상행동을 출력하지 않는다.
- [ ] legacy_unverified 누출 0건이다.
- [ ] 동일 입력 결정성 100%, 계보 완전성 100%다.
