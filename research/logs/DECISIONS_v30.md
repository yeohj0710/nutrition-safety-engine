...
## 데모 예시를 실제 근거 분포에 맞춰 재구성

사용자가 공개 화면의 입력 예시가 데모로 적절한지 물었다. 확인해 보니 15개 예시의 용량이
내가 걷어낸 선행 트랙 하드코딩 문자열의 숫자(오메가-3 2,000 mg/day, 칼슘 600 mg/day)와
정확히 일치했다. 그 서술이 그럴듯해 보이도록 맞춰 둔 예시였다.

별칭별 후보 근거를 원문까지 훑어 보니 문제가 더 구체적이었다.

- A1 비타민 K 7건 — 와파린과 비타민 K 상호작용 메타분석, 와파린 복용자 허용 섭취량 고찰,
  와파린-보충제 상호작용 체계적 고찰. 주제가 정확히 맞는다
- A2 오메가-3 1건 — 심혈관약-보충제 상호작용 고찰. 스타틴·와파린·항혈소판제를 다룬다
- B1 칼슘 1건 · B2 비타민 D 2건 · B3 비타민 C 5건 — **전부 만성콩팥병·투석 문헌**이다.
  별칭 B1~B3 이 HRS2_KIDNEY_DISEASE 에 연결돼 있기 때문이다. 결석·옥살산 문헌은 없다

즉 기존 예시의 결석·요중 옥살산 프레이밍은 근거가 뒷받침하지 않는 입력이었다. 실제로 돌려
보니 `calcium-stone-high-urine-calcium` 과 `vitamin-d-upper-limit-stone` 은 개인화 신호가
0개였고, 선택 사유가 "체계적 문헌고찰입니다" 한 줄뿐이었다.

후보 입력을 실제 응답으로 측정해 고쳤다. 신호 수는 설계 유형을 제외한 선택 사유 개수다.

| 예시 | 이전 | 이후 |
|---|---|---|
| 오메가-3 (아픽사반 → 와파린) | direct 0 | **direct 1** |
| 칼슘 (결석 → 만성콩팥병) | 신호 0 | **신호 2** |
| 비타민 D (결석 → 만성콩팥병·복막투석) | 신호 0 | **신호 2~3** |
| 비타민 C (옥살산 → 신장기능 저하·철분제) | 신호 1 | **신호 4~5** |
| 비타민 K | 신호 5~11 | 그대로 유지 |

표시 순서도 후보 근거 수를 따라 비타민 K(7) → 비타민 C(5) → 비타민 D(2) → 오메가-3(1) →
칼슘(1) 으로 바꿨다. 첫 화면에서 근거가 실제로 붙는 사례를 먼저 보여주기 위해서다.
폼의 보충제 버튼 순서도 같은 배열을 쓰도록 통일했고, "입력 예시 15개" 라벨은 배열 길이에서
읽도록 바꿔 개수가 어긋나지 않게 했다.

신호가 0인 예시 3개(`vitamin-c-low-dose-no-risk`, `calcium-levothyroxine`,
`calcium-unknown-antibiotic`)는 그대로 뒀다. 위험 조건이 없거나 용량을 모르는 사례이므로
개인화 신호가 발화하지 않는 것이 옳은 동작이다.

한계는 남는다. 예시를 바꿔도 A2·B1 의 후보 근거는 각각 1건이고, B 계열 근거에는 결석·옥살산
문헌이 없다. 이건 검색식을 넓혀야 풀리는 문제이고 할일에 남아 있다.
