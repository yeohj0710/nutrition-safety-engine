...
import json,hashlib
R=Path(__file__).resolve().parents[1];d=R/"research/thesis/systematic_review_thesis_v3.docx";p=R/"research/thesis/systematic_review_thesis_v3.pdf";m=json.loads((R/"research/systematic_review_v3/manifest.json").read_text(encoding="utf-8"));c=json.loads((R/"research/systematic_review_v3/core_manifest.json").read_text(encoding="utf-8"))
doc=Document(d);pdf=PdfReader(p);text="\n".join(x.text for x in doc.paragraphs)+"\n"+"\n".join(x.extract_text() or "" for x in pdf.pages);errors=[]
for x in ["체계적 근거 검토","개인맞춤 조회 시스템",f"{m['records']:,}",f"{m['with_dose']:,}",f"{m['with_fulltext_locator']:,}",str(c['core_records']),"https://nutrition-safety-engine.vercel.app","59개 자동 테스트"]:
