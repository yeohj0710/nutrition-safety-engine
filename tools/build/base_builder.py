...
def _load_query_terms() -> dict[str, dict[str, list[re.Pattern[str]]]]:
    """query_definitions.json의 P/I 검색어를 제목·초록 정규식으로 준비한다."""
    payload = json.loads(PICOS_PATH.read_text(encoding="utf-8"))
    result: dict[str, dict[str, list[re.Pattern[str]]]] = {}
    for question in payload["questions"]:
        result[question["question_id"]] = {}
        for field in ("P_terms", "I_terms"):
            patterns: list[re.Pattern[str]] = []
            for raw_term in question[field]:
                term = re.sub(r"\[[^\]]+\]$", "", str(raw_term).strip()).strip('"')
                chunks = term.split("*")
                pattern = "\\w*".join(re.escape(chunk) for chunk in chunks)
                patterns.append(re.compile(
                    rf"(?<![A-Za-z0-9]){pattern}(?![A-Za-z0-9])", re.IGNORECASE
                ))
            result[question["question_id"]][field] = patterns
    return result


QUERY_TERM_PATTERNS = _load_query_terms()
...
def legacy_priority_score(row: dict[str, str], key_finding: str) -> int:
...
def topic_match_counts(row: dict[str, str]) -> dict[str, int]:
    text = f"{row['title']} {row['abstract']}"
    patterns = QUERY_TERM_PATTERNS[row["question_id"]]
    return {
        "p_hits": sum(bool(pattern.search(text)) for pattern in patterns["P_terms"]),
        "i_hits": sum(bool(pattern.search(text)) for pattern in patterns["I_terms"]),
    }


def topic_relevant(row: dict[str, str]) -> bool:
...
def extract_observed_axes(row: dict[str, str]) -> list[str]:
    """보고한 축만 켠다. 일반 명사 하나만으로 약·병 축을 켜지 않는다."""
    text = f"{row['title']} {row['abstract']}"
    axes: list[str] = []
    if re.search(r"child|adolesc|adult|elder|older|aged|years? old|age[sd]?\b", text, re.IGNORECASE):
        axes.append("age_group")
    if re.search(r"\bmen\b|\bwomen\b|\bmale\b|\bfemale\b|\bsex\b|\bgender\b", text, re.IGNORECASE):
        axes.append("sex")
    if re.search(
        r"warfarin|apixaban|rivaroxaban|dabigatran|edoxaban|heparin|enoxaparin|"
        r"aspirin|clopidogrel|ticagrelor|prasugrel|anticoagulant|antiplatelet|"
        r"concomitant\s+(?:medication|drug)|drug\s+interaction|medication\s+use|co-medication",
        text,
        re.IGNORECASE,
    ):
        axes.append("concomitant_medication")
    if re.search(
        r"chronic\s+kidney\s+disease|kidney\s+(?:disease|failure)|renal\s+(?:disease|failure|insufficiency)|"
        r"dialysis|end-stage\s+renal|pregnan|pregnancy|liver\s+disease|hepatic\s+disease|"
        r"cirrho|hepatitis|diabetes\s+mellitus|diabetic|hypertension|hypertensive|"
        r"cancer|malignan|neoplasm|carcinoma",
        text,
        re.IGNORECASE,
    ):
        axes.append("underlying_condition")
