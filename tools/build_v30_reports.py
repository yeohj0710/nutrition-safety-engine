...
CLAIMS = [
    ...
    },
    {
        ...
        "evidence": (
            ...
            f"{RM['track']} 로 고정된다. 공개 화면의 연구 서술은 상수 문자열이 아니라 각 응답이 "
            f"실제로 선택한 근거에서 생성하며, 인용문이 선택된 근거 원문에 그대로 존재하는지를 "
            f"회귀 테스트가 검사한다. 다만 별칭별 후보 근거 수가 적은 질문이 있어 근거 폭은 "
            f"선행 트랙보다 좁고, 그런 경우 서술은 문헌이 부족하다는 사실을 그대로 적는다."
        ),
        "evidence_paths": [
            rel(P["core_manifest"]),
            rel(P["validation"]),
...
slides.append(
    (
        "11. 다음 단계",
        [
            "바로 다음에 할 일은 정해져 있습니다. 채점을 다른 회사의 인공지능에게 한 번 더 "
            "시키는 것입니다. 지금은 문헌을 고른 쪽과 채점한 쪽이 같아서, 같은 자리에서 같은 "
            "방향으로 틀려도 알 수가 없습니다.",
            "채점 분량은 300건이라 문헌을 고른 2,209건보다 훨씬 작습니다. 그래서 작은 쪽만 "
            "넘기면 됩니다. 넘길 자료와 받은 결과를 검사할 도구는 이미 만들어 뒀습니다.",
            "그렇게 하면 지금 쓸 수 없는 일치도 숫자가 '서로 다른 두 평가자가 얼마나 같게 "
            "봤는가'로 바뀝니다. 그게 원래 보고 싶었던 숫자입니다.",
            "그 다음이 사람 두 명 이상이 같은 표본을 독립적으로 판정한 정답지입니다. "
...
notion_lines += [
    ...
    "## 다음 단계",
    "",
    "1. **참조표준 채점을 다른 모델 계열(Codex)이 수행하는 교차검증 arm 을 실행한다. 인계 패키지와 "
    "검증 도구는 준비 완료.** 채점은 300건으로 선별 2,209건보다 작아 넘기기 쉽고, 참조표준이야말로 "
    "분류기와 독립이어야 하는 쪽이다. 실행하면 지금 인용할 수 없는 재검사 일치도가 평가자 간 "
    "일치도로 바뀐다. 인계 지시서 `research/validation/screening_ai_reference_v3/codex_arm/README.md`, "
    "검증·비교 도구 `tools/v30/codex_reference_arm.py`.",
    "2. 사람 2인 이상이 같은 층화 표본을 독립 판정해 기준 정답을 만든다. 그 뒤에야 민감도·특이도를 "
    "임상적 의미로 해석할 수 있다.",
    "3. 자료원을 PubMed 밖으로 확장한다.",
    "4. 원문 접근이 가능한 부분집합에서 초록 기반 판정과 원문 기반 판정의 차이를 측정한다.",
    "5. 별칭별 근거 폭이 좁은 질문을 확인하고 검색식 확장 여부를 판단한다. 데모 예시도 후보 근거가 "
    "많은 조합으로 다시 고른다.",
    "",
]
...
UNRESOLVED = [
    ...
    },
    {
        ...
        "evidence_path": "research/synthesis/screener_vs_ai_reference_v3.json",
        "handled": (
            "해소 경로를 준비해 뒀다. 참조표준 채점(300건)만 다른 모델 계열에 넘기는 교차검증 "
            "arm 이며, 선별(2,209행)보다 작고 독립이 필요한 쪽이 참조표준이라 역할 분리가 맞다. "
            "인계 패키지 research/validation/screening_ai_reference_v3/codex_arm/, 검증·비교 도구 "
            "tools/v30/codex_reference_arm.py. 실행 전이므로 아직 미해소다."
        ),
        "prepared_next_step": {
            "arm": "codex_reference_cross_check",
            "handoff": "research/validation/screening_ai_reference_v3/codex_arm/README.md",
            "tool": "tools/v30/codex_reference_arm.py",
            "workload_rows": 300,
            "status": "prepared_awaiting_external_run",
        },
    },
    {
