...
if d[["record_id","title","authors","venue","source_url","population_evidence","outcome_evidence","evidence_locator","extraction_authority"]].astype(str).apply(lambda x:x.str.strip().eq("")).any().any():errors.append("required evidence field empty")
...
for _,r in d.iterrows():
 ...
 if re.search(r"\b(infant\w*|child\w*|pediatric|paediatric|adolescen\w*|juvenile|neonat\w*|newborn|pregnan\w*|mouse|mice|murine|rat|rats|canine|dog|dogs|frog|frogs|bovine|swine|pig|pigs|chick\w*|poultry|rabbit\w*|sheep|lamb\w*|monkey\w*|hamster\w*|guinea pig\w*)\b|first year of life",scope_text):bad.append(r.record_id)
