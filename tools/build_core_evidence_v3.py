...
for q,g in core.groupby("question_id"):
 cites=[{"record_id":x.record_id,"title":x.title,"authors":x.authors,"venue":x.venue,"year":int(float(x.year)) if x.year!="" else None,"doi":x.doi,"url":x.source_url,"locator":x.evidence_locator,"dose":x.dose_extracted,"outcome":x.outcome_evidence} for _,x in g.head(5).iterrows()]
