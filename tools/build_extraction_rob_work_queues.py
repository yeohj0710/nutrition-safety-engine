...
def main() -> int:
    ...
    write(EXTRACTION_QUEUE, ["extraction_task_id", *common, "extractor_id", "verifier_id", "started_at", "completed_at", "status"], extraction_rows)
    write(ROB_QUEUE, ["rob_task_id", *common, "tool_name", "tool_route_status", "tool_version", "reviewer_1_id",
                      "reviewer_2_id", "adjudicator_id", "started_at", "completed_at", "status"], rob_rows)
    ...
    contract = {"schema_version": "1.0.0", "status": "awaiting_verified_fulltext_inclusions",
                "included_fulltext_rows": len(included), "extraction_work_rows": len(extraction_rows),
                "rob_work_rows": len(rob_rows), "human_extractions": 0, "human_rob_assessments": 0,
